Shader "Game2DWaterKit/Lightweight Render Pipeline/Lit/Waterfall"
{
	Properties
	{
		// Body Properties
	...
	SubShader
	{
		...
		Pass
		{
			...
			// Unity LWRP -> URP automatic upgrade process will throw a "failed to open source file ../CombindedShapeLightShared.hlsl" due to a typo in the source filename ("CombindedShapeLightShared.hlsl" instead of "CombinedShapeLightShared.hlsl")
			// so we'll avoid this error and include the file with the correct name if the UNIVERSAL_PIPELINE_CORE_INCLUDED is defined
			...
			half4 frag (Varyings i) : SV_Target
			{
				...
				return CombinedShapeLightShared(c, 0, i.lightingUV);
			}
	...
	Fallback "Game2DWaterKit/Lightweight Render Pipeline/Unlit/Waterfall"
